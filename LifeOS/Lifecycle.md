...
[[Library/LifeOS/Signals]], computed and never stored.
...
-- priority: 10
lifeos = lifeos or {}
lifeos.lifecycle = lifeos.lifecycle or {}

lifeos.lifecycle.transitions = {
  { command = "LifeOS: Pause Project", to = "paused",
    from = { "active" } },
  { command = "LifeOS: Complete Project", to = "completed",
    from = { "active", "paused" } },
  { command = "LifeOS: Archive Project", to = "archived",
    from = { "active", "paused", "completed" } },
  { command = "LifeOS: Reactivate Project", to = "active",
...
function lifeos.lifecycle.setStatus(pageName, target, allowedFrom)
  ...
  lifeos.writePageText(pageName, index.patchFrontmatter(text, {
    { op = "set-key", path = "status", value = target },
  }))
  return true, target
end

for _, transition in ipairs(lifeos.lifecycle.transitions) do
  command.define {
    name = transition.command,
    run = function()
      local ok, message = lifeos.lifecycle.setStatus(
