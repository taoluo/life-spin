...
export async function capture(vault: Vault, line: string, page = INBOX_PAGE): Promise<MutationResult<{ line: string }>> {
  ...
  if (vault.exists(path)) cs.expected.set(path, text);
...
export async function makeTask(
  ...
  const task = first.replace(/^(\s*(?:[-*+]|\d+[.)]))\s+/, "$1 [ ] ");
